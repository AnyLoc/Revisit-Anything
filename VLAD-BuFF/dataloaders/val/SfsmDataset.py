...
DATASET_ROOT = "/work/qvpr/data/raw/sf_xl/small/test/"
GT_ROOT = (
...
if not path_obj.exists():
    raise Exception(
        f"Please make sure the path {DATASET_ROOT} to sfsm dataset is correct"
    )

if not path_obj.joinpath("database") or not path_obj.joinpath("queries_v1"):
    raise Exception(
        f"Please make sure the directories queries and database are situated in the directory {DATASET_ROOT}"
    )


class SfsmDataset(Dataset):
    def __init__(self, input_transform=None, positive_dist_threshold=25):
        self.input_transform = input_transform
        self.dataset_root = DATASET_ROOT

        # reference images names
        self.dbImages = np.load(GT_ROOT + "sfsm/sfsm_dbImages.npy")

        # query images names
        self.qImages = np.load(GT_ROOT + "sfsm/sfsm_qImages.npy")
