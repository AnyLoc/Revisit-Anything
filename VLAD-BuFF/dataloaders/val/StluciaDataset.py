...
DATASET_ROOT = "./datasets_vg/datasets/st_lucia/images/test/"
GT_ROOT = (
...
if not path_obj.exists():
    raise Exception(
        f"Please make sure the path {DATASET_ROOT} to st_lucia dataset is correct"
    )
...
class StluciaDataset(Dataset):
    def __init__(self, input_transform=None, positive_dist_threshold=25):
        ...
        self.dbImages = np.load(GT_ROOT + "st_lucia/st_lucia_dbImages.npy")

        # query images names
        self.qImages = np.load(GT_ROOT + "st_lucia/st_lucia_qImages.npy")
